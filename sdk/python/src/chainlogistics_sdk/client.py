...
class ChainLogisticsClient:
    ...
    def _cache_key(self, method: str, path: str, params: Optional[Dict[str, Any]]) -> str:
        serialized_params = ""
        if params:
            serialized_params = "&".join(
                f"{key}={params[key]}" for key in sorted(params.keys())
            )
        return f"{method.upper()}:{path}:{serialized_params}"
    ...
    def readiness_check(self):
        ...
        data = self.get("health/ready")
        return ReadinessResponse(**data)

    def liveness_check(self):
        """Perform a liveness check."""
        data = self.get("health/live")
        return LivenessResponse(**data)

    def monitoring_health(self):
        """Retrieve health monitoring details."""
        data = self.get("health/monitoring")
    ...
    def batch_get(
        self,
        requests_to_make: Iterable[Tuple[str, Optional[Dict[str, Any]]]],
        max_workers: int = 4,
    ) -> Dict[str, Any]:
        """Execute multiple GET requests concurrently."""

        def fetch(item: Tuple[str, Optional[Dict[str, Any]]]) -> Tuple[str, Any]:
            path, params = item
            return path, self.get(path, params=params)

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            results = list(executor.map(fetch, requests_to_make))

        return {path: value for path, value in results}
