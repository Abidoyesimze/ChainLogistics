...
impl ChainLogisticsClient {
    ...
    pub async fn health_check(&self) -> Result<crate::models::HealthResponse> {
        let request = HttpClient::new(self.client.clone(), self.config.clone()).get("health");
        HttpClient::new(self.client.clone(), self.config.clone())
            .execute(request)
            .await
    }

    /// Perform a readiness check
    pub async fn readiness_check(&self) -> Result<crate::models::ReadinessResponse> {
        let http = HttpClient::new(self.client.clone(), self.config.clone());
        let request = http.get("health/ready");
        http.execute(request).await
    }

    /// Perform a liveness check
    pub async fn liveness_check(&self) -> Result<crate::models::LivenessResponse> {
        let http = HttpClient::new(self.client.clone(), self.config.clone());
        let request = http.get("health/live");
        http.execute(request).await
    }

    /// Retrieve monitoring health details
    pub async fn monitoring_health(&self) -> Result<crate::models::MonitoringHealthResponse> {
        let http = HttpClient::new(self.client.clone(), self.config.clone());
        let request = http.get("health/monitoring");
        http.execute(request).await
    }
    ...
    pub async fn db_health_check(&self) -> Result<crate::models::DbHealthResponse> {
        let http = HttpClient::new(self.client.clone(), self.config.clone());
        let request = http.get("health/db");
        http.execute(request).await
    }

    /// Batch key health probes to reduce request orchestration overhead.
    pub async fn batch_health_checks(
        &self,
    ) -> Result<(
        crate::models::HealthResponse,
        crate::models::ReadinessResponse,
        crate::models::LivenessResponse,
    )> {
        let health = self.health_check();
        let readiness = self.readiness_check();
        let liveness = self.liveness_check();

        let (health, readiness, liveness) = tokio::try_join!(health, readiness, liveness)?;
